...
@lexer::members {
	...
	private IndentStack _indents = new IndentStack();
	...
	void emitEndToken(Token token) {
		this._tokens.enq(token);
	}

	void emitIndent(int length) {
		Token t = createToken(Python2Parser.INDENT, "INDENT", length);
		_tokens.enq(t);
	}

	void emitDedent(){
		    Token t = this.createToken(Python2Parser.DEDENT, "DEDENT",null);
		    this._tokens.enq(t);
		}
	...
	void emitEndmarker() {
		Token t = this.createToken(Python2Parser.ENDMARKER, "ENDMARKER",null);
		this._tokens.enq(t);
	}

	void emitNewline() {
		Token t = this.createToken(Python2Parser.NEWLINE, "NEWLINE",null);
		this._tokens.enq(t);
	}

	Token createToken(int type_, String text, Integer length) {
		if (length == null)
			length = 0;
		int start = this._tokenStartCharIndex;
		int stop = start + length;
		Token t = new CommonToken(this._tokenFactorySourcePair, type_, this.DEFAULT_TOKEN_CHANNEL, start, stop);
		return t;
	}
}
...
WHITESPACE: ('\t' | ' ')+
{
if (this._tokenStartCharPositionInLine == 0 && this._openBRCount == 0 && ! this._lineContinuation) {
